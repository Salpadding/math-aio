...
\begin{proof}
    ...
    By $I_{x_0} \subseteq A$, we got $x_0 \in A$, which is contradict with $x_0 \in X \setminus A$.
\end{proof}

\begin{example}
    Consider $X = \{ 0,1,2, \dots \}$ and $A = \{ 1,2,3, \dots \}$. 

    The $A$ does not meets $\forall x \in X, I_x \subseteq A \Rightarrow x \in A$. Because $I_0$
...
\begin{proof}
    ...
    If $J \ne X$, let $b = \min X \setminus J$, $B = \{ x \in X: b \le x \}$. We will prove $X \setminus J = B$.
